...
// VWorld 한글 라벨(Hybrid: 지명/도로/경계, 리 단위까지). 키 없으면 Esri 영문 라벨로 폴백
const satLabels = VKEY
  ? L.tileLayer('https://api.vworld.kr/req/wmts/1.0.0/'+VKEY+'/Hybrid/{z}/{y}/{x}.png', {maxZoom:SAT_MAXZOOM, maxNativeZoom:SAT_MAXZOOM, pane:'satLabelsPane'})
  : L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}', {maxZoom:SAT_MAXZOOM, maxNativeZoom:SAT_MAXZOOM, pane:'satLabelsPane'});
const baseSat = L.layerGroup([satImg, satLabels]);     // 위성+한글지명 하이브리드
...
// 위성지도일 때 최대 줌 제한(고배율 미제공 → 더 확대 안 되고 멈춤). 일반지도면 복원
