...
<style>
  ...
  .cafecard{display:flex;align-items:center;gap:10px;background:#fff;padding:8px 13px 8px 8px;border-radius:14px;box-shadow:0 4px 16px rgba(0,0,0,.2);text-decoration:none}
...
function addPlaceMarker(pl){ const c=placeColors(pl.cat);
  L.circleMarker([pl.lat,pl.lng],{radius:c.r,color:c.line,weight:1,fillColor:c.fill,fillOpacity:.95}).addTo(regLayer)
   .on('click',function(){ openPlaceModal(pl); }); }
...
Object.keys(_courseGroups).forEach(function(sc){
  ...
  const casing=L.geoJSON(fc,{style:{color:'#2a0a4a',weight:8,opacity:0.55}});
...
legend.onAdd=function(){ const d=L.DomUtil.create('div','legend');
  d.innerHTML='<b>범례</b>'+
    ...
    (COURSES.features.length?'<br><span class="sw" style="width:16px;height:4px;background:#7c4dff"></span>카누잉코스':'')+
