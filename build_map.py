...
function suggestPlace(){
  ...
  document.getElementById('sgBody').innerHTML=
    ...
    +'<div class="seg" id="sgSeg"><button type="button" class="seg-b on" data-v="런칭/랜딩">🛶 런칭/랜딩</button><button type="button" class="seg-b" data-v="장애물">🚧 장애물</button><button type="button" class="seg-b" data-v="기타">📍 기타</button></div>'
...
const ObstacleCtl=L.Control.extend({ options:{position:'topleft'},
  onAdd:function(){ const d=L.DomUtil.create('div','measbtn obsbtn'); d.id='obsBtnBox'; d.innerHTML='🚧 장애물'; d.title='장애물 표시 추가(관리자)'; d.style.display='none';
    ...
    return d; } });
// (버튼 추가는 공지 버튼 다음 — 거리측정/공지/장애물 순서)
...
const famousLayer = L.geoJSON(spotFeats, {
  ...
  onEachFeature:(f,l)=>{ const p=f.properties||{}; l.on('click',function(){ openPlaceModal(featPlace(f)); });
    ...
    if(p.id!=null) _placeMarkerById[p.id]={m:l,cat:'spot',name:p.name||''}; }
}).addTo(map);
const canoeLayer = L.geoJSON(landFeats, {
  pointToLayer:(f,ll)=>makeMarker(ll, isWreck((f.properties||{}).name)?'wreck':'canoe'),
  onEachFeature:(f,l)=>{ const p=f.properties||{}; l.on('click',function(){ openPlaceModal(featPlace(f)); });
    const c=f.geometry.coordinates; _favReg(placeSlug(c[1],c[0]),l,null);
    if(p.id!=null) _placeMarkerById[p.id]={m:l,cat:'canoe',name:p.name||''}; }
}).addTo(map);
...
function renderPmCat(pl){   // 관리자 전용: 장소 분류(명소/런칭·랜딩) 토글
  ...
  el.innerHTML='<span class="pm-catlbl">분류(관리자)</span>'
    +'<a data-k="spot" class="pm-cat'+(cur==='spot'?' on':'')+'">📍 명소</a>'
...
function _localSearch(q){
  ...
  POINTS.features.forEach(function(f){ const p=f.properties||{}; const nm=p.name||'';
...
map.addControl(new ObstacleCtl());   // 장애물(관리자) — 공지 아래
