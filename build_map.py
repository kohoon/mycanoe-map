...
<style>
  ...
  .legend-c .lg-h{cursor:pointer;font-weight:700;white-space:nowrap;user-select:none}
  .legend-c .lg-ar{display:inline-block;transition:transform .2s}
  .legend-c.open .lg-ar{transform:rotate(180deg)}
  .legend-c .lg-b{display:none;margin-top:6px}
  .legend-c.open .lg-b{display:block}
...
function addPlaceMarker(pl){ const spot=(pl.cat==='명소');
  ...
  m.addTo(spot?famousLayer:canoeLayer); }
...
const famousLayer = L.geoJSON(spotFeats, {
  pointToLayer:(f,ll)=>L.marker(ll,{icon:boathouseIcon()}),
...
legend.onAdd=function(){ const d=L.DomUtil.create('div','legend legend-c');
  d.innerHTML='<div class="lg-h">범례 <span class="lg-ar">▾</span></div><div class="lg-b">'+
    ...
    '</div>';
  L.DomEvent.disableClickPropagation(d);
  const h=d.querySelector('.lg-h'); L.DomEvent.on(h,'click',function(){ d.classList.toggle('open'); });
  return d; };
legend.addTo(map);
