...
function loadCourses(){ fetch(courseReadUrl('/courses')).then(function(r){return r.json();}).then(function(list){ (list||[]).forEach(renderKVCourse); _courseKvReady=true; _applyCourseFocus(); _maybeSyncAdminCourseFavs(); }).catch(function(){ _courseKvReady=true; _applyCourseFocus(); _maybeSyncAdminCourseFavs(); }); }
// 정적 코스 숨김(관리자 삭제분) 적용
...
function focusCourseFromUrl(){
  ...
  if(id.charAt(0)==='k'){ const kid=id.slice(1);   // KV 코스: 별도 조회
    fetch(courseReadUrl('/courses')).then(function(r){return r.json();}).then(function(list){ const c=_findSharedKvCourse(list,kid); if(c){
