...
async function waterRoute(p1,p2){
  ...
  const q='[out:json][timeout:25];(way["waterway"~"river|stream|canal"]('+s+','+w+','+n+','+e+');way["natural"="water"]('+s+','+w+','+n+','+e+');relation["natural"="water"]('+s+','+w+','+n+','+e+'););out geom;';
  ...
  for(const el of (j.elements||[])){
    if(el.type==='way'&&el.tags&&el.tags.natural==='water'&&el.geometry&&el.geometry.length>2) addWater(waterOuter,el.geometry.map(function(p){return [p.lat,p.lon];}));
    if(el.type==='relation'&&el.tags&&el.tags.natural==='water') (el.members||[]).forEach(function(m){if(m.geometry&&m.geometry.length>2)addWater(m.role==='inner'?waterInner:waterOuter,m.geometry.map(function(p){return [p.lat,p.lon];}));});
    if(el.type!=='way'||!el.tags||!el.tags.waterway||!el.geometry) continue; let prev=null;
  ...
  function plausibleWater(pt){if(inWater(pt))return true;let d=1e18;for(const x of waterOuter)d=Math.min(d,edgeDist(pt,x));for(const x of waterInner)d=Math.min(d,edgeDist(pt,x));return d<=180;}
