...
<style>
  ...
  .river-focus-bar{display:none;align-items:center;gap:8px;max-width:min(260px,65vw);box-sizing:border-box;background:#003b73;color:#fff;padding:8px 8px 8px 11px;border-radius:6px;box-shadow:0 2px 8px rgba(0,0,0,.38);font:700 13px/1.2 sans-serif}
  .river-focus-bar.on{display:flex}
  ...
  .measbtn{cursor:pointer;font:600 13px/1.25 sans-serif;background:#fff;color:#222;padding:7px 10px;border-radius:6px;box-shadow:0 1px 4px rgba(0,0,0,.3);white-space:nowrap;user-select:none;width:112px;box-sizing:border-box;text-align:center}
  .measbtn .meas-sub{font-size:11px;font-weight:500;color:#667}
  ...
  #obName,#obKakaoUrl{width:100%;box-sizing:border-box;padding:9px;border:1px solid #ccd;border-radius:9px;font-size:13.5px;margin-bottom:10px}
  ...
  .wayback-compare{display:none;position:absolute;inset:0;z-index:650;pointer-events:none}
  .wayback-compare.on{display:block}
  .wayback-divider{position:absolute;left:50%;top:0;bottom:0;width:2px;transform:translateX(-1px);background:rgba(255,255,255,.92);box-shadow:0 0 0 1px rgba(0,0,0,.35),0 0 8px rgba(0,0,0,.45)}
  .wayback-side{position:absolute;top:12px;transform:translateX(-50%);padding:6px 10px;border-radius:6px;background:rgba(20,31,42,.84);color:#fff;font:700 12px/1.2 sans-serif;box-shadow:0 2px 7px rgba(0,0,0,.28);white-space:nowrap}
  .wayback-now{left:25%}
  .wayback-old{left:75%}
  ...
  .cm-ana-bad{background:#fff4e5;color:#8a4b00}
  ...
  .addform .aprow{font:12px sans-serif;margin-bottom:8px;display:grid;grid-template-columns:1fr 1fr;gap:6px}
  .addform .aprow label{display:flex;align-items:center;gap:6px;line-height:1.1;padding:8px 8px;border:1px solid #d6dee2;border-radius:8px;background:#fff;cursor:pointer}
  .addform .aprow label:last-child{grid-column:1/-1}
  .addform .aprow input{margin:0}
  .addform .aprow strong{display:block;font-weight:700}
  .addform #apSeg{display:grid;grid-template-columns:1fr 1fr;gap:6px}
  .addform #apSeg .seg-b{min-width:0;padding:8px 7px;font-size:12px;line-height:1.1;white-space:normal}
  .addform #apSeg .seg-b[data-v="candidate"]{grid-column:1/-1}
  .addform #apSeg .seg-b span{display:inline-block}
  .addform #apMemo{width:100%;box-sizing:border-box;padding:8px 9px;margin:0 0 10px;border:1px solid #bbb;border-radius:5px;font-size:13px;resize:vertical;min-height:66px;line-height:1.35;background:#fff}
  ...
  .canoe-pin-in.candidate{background:#ff9800}
  ...
  .dot-candidate{background:#ff9800}
  ...
  .pm-note-warn{background:#fff4e5;color:#8a4b00;border:1px solid #ffd59e}
  .course-actions{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px}
  .course-btn{display:inline-flex;align-items:center;justify-content:center;gap:5px;border:1px solid #cbd8e5;background:#f7fbff;color:#154b86;border-radius:999px;padding:7px 11px;font:700 12px sans-serif;cursor:pointer;text-decoration:none;line-height:1}
  .course-btn.on{background:#1565c0;color:#fff;border-color:#1565c0}
  .course-btn.ghost{background:#fff;color:#455a64}
  .course-btn.danger{background:#fff;color:#c62828;border-color:#efb4b4}
  .course-btn.danger.on{background:#c62828;color:#fff;border-color:#c62828}
  .pm-padmin{display:flex;flex-wrap:wrap;gap:6px}
  .pm-cat a[data-k="candidate"]{border-color:#ffb74d;color:#ef6c00}
  .pm-cat a[data-k="candidate"].on{background:#ff9800;color:#fff;border-color:#ff9800}
  ...
  #tripbar.paused .tb-start{animation:none}
  .tb-pause,.tb-refresh{display:none;background:#fff;color:#263238;border:0;border-radius:24px;padding:12px 14px;font:700 13px sans-serif;box-shadow:0 3px 12px rgba(0,0,0,.25);cursor:pointer;white-space:nowrap}
  #tripbar.rec .tb-pause,#tripbar.rec .tb-refresh{display:inline-block}
...
let _wbVersions=null, _wbLayer=null, _wbTarget=null, _wbLoadSeq=0;
const _wbLocalCache={};
...
function _wbTileAt(d,z){
  const n=Math.pow(2,z), lat=Math.max(-85.0511,Math.min(85.0511,+d.lat||0)), lng=+d.lng||0;
  return {x:Math.floor((lng+180)/360*n),y:Math.floor((1-Math.asinh(Math.tan(lat*Math.PI/180))/Math.PI)/2*n)};
}
async function _wbLocalVersions(rows,d,onProgress){
  const key=String(d.cd||'')+'|'+Number(d.lat).toFixed(4)+'|'+Number(d.lng).toFixed(4);
  if(_wbLocalCache[key]) return _wbLocalCache[key];
  const z=14, p=_wbTileAt(d,z), sigs=new Array(rows.length);
  let next=0, done=0;
  async function worker(){
    while(next<rows.length){
      const i=next++, v=rows[i];
      const u='https://wayback.maptiles.arcgis.com/arcgis/rest/services/World_Imagery/WMTS/1.0.0/default028mm/MapServer/tile/'+v.id+'/'+z+'/'+p.y+'/'+p.x;
      try{
        const r=await fetch(u,{redirect:'follow'}), b=await r.arrayBuffer();
        const h=await crypto.subtle.digest('SHA-256',b);
        sigs[i]=Array.from(new Uint8Array(h)).map(function(x){return x.toString(16).padStart(2,'0');}).join('');
      }catch(e){ sigs[i]=v.id; }
      done++; if(onProgress) onProgress(done,rows.length);
    }
  }
  await Promise.all(Array.from({length:8},worker));
  const changed=[];
  for(let i=0;i<rows.length;){
    let j=i; while(j+1<rows.length&&sigs[j+1]===sigs[i]) j++;
    changed.push(rows[j]);   // 같은 영상이 이어진 배포본 중 실제로 처음 등장한 날짜
    i=j+1;
  }
  _wbLocalCache[key]=changed;
  return changed;
}
function _wbDraw(){
  if(!_wbTarget) return;
  const sel=document.getElementById('waybackDate');
  ...
  _wbLayer=L.tileLayer(url,{bounds:_wbBounds(_wbTarget),maxNativeZoom:18,maxZoom:19,opacity:1,attribution:'Historical imagery © Esri Wayback'}).addTo(map);
  ...
  const old=document.querySelector('#waybackCompare .wayback-old');
  if(old) old.textContent='과거 · '+(sel.options[sel.selectedIndex]?sel.options[sel.selectedIndex].text.replace(' 배포본',''):'선택 시점');
  const ui=document.getElementById('waybackCompare'); if(ui) ui.classList.add('on');
  try{ localStorage.setItem('mc_wayback_id',sel.value); }catch(e){}
}
function _wbClip(){
  if(!_wbLayer) return;
  const c=_wbLayer.getContainer(); if(!c) return;
  const sz=map.getSize(), nw=map.containerPointToLayerPoint([0,0]), se=map.containerPointToLayerPoint([sz.x,sz.y]);
  const mid=map.containerPointToLayerPoint([Math.round(sz.x/2),0]).x;
  c.style.clip='rect('+nw.y+'px '+se.x+'px '+se.y+'px '+mid+'px)';
}
...
function _openWayback(cd){
  ...
  let ui=document.getElementById('waybackCompare');
  if(!ui){ ui=L.DomUtil.create('div','wayback-compare',map.getContainer()); ui.id='waybackCompare';
    ui.innerHTML='<div class="wayback-side wayback-now">현재 · '+(_satSrc==='vworld'?'VWorld':'Esri')+'</div><div class="wayback-divider"></div><div class="wayback-side wayback-old">과거 위성영상</div>'; }
  ...
  _wbLoadVersions().then(function(rows){
    if(loadSeq!==_wbLoadSeq) return [];
    sel.innerHTML='<option>해당 위치 변경일 분석 중…</option>';
    return _wbLocalVersions(rows,d,function(done,total){ if(loadSeq===_wbLoadSeq) sel.innerHTML='<option>해당 위치 변경일 분석 중… '+done+'/'+total+'</option>'; });
  }).then(function(rows){
    if(loadSeq!==_wbLoadSeq||!rows.length) return;
  ...
  }).catch(function(){ if(loadSeq===_wbLoadSeq) sel.innerHTML='<option>영상 목록을 불러오지 못했습니다</option>'; });
...
function extLinks(lat,lng,label,hasRv){   // hasRv===false 면 로드뷰 링크 숨김(없는 곳)
  ...
  if(isAndroid){
    ...
    n='intent://place?lat='+lat+'&lng='+lng+'&name='+enc+'&appname=canoe.crowdbase.kr#Intent;scheme=nmap;package=com.nhn.android.nmap;S.browser_fallback_url='+encodeURIComponent('https://map.naver.com/p/search/'+lat+','+lng)+';end';
    ...
    n='nmap://place?lat='+lat+'&lng='+lng+'&name='+enc+'&appname=canoe.crowdbase.kr';
...
function addPlace(){
  ...
  const html='<div class="addform"><b>장소 등록</b>'
    ...
    +'<div class="sg-label">분류 선택</div><div class="seg" id="apSeg">'
    +'<button type="button" class="seg-b" data-v="명소">📍 <span>카누명소</span></button>'
    +'<button type="button" class="seg-b on" data-v="런칭랜딩">🛶 <span>런칭<br>랜딩</span></button>'
    +'<button type="button" class="seg-b" data-v="candidate">🟠 <span>런칭<br>랜딩 후보지</span></button></div>'
    +'<div class="sg-label">내용(메모)</div>'
    +'<textarea id="apMemo" rows="2" maxlength="500" placeholder="예: 진입로 좁음 / 주차 가능 / 현장 확인 필요"></textarea>'
  ...
  setTimeout(function(){ const sv=document.getElementById('apSave'); if(!sv) return;
    const seg=document.getElementById('apSeg'); if(seg) seg.onclick=function(e){ const t=e.target.closest('.seg-b'); if(!t) return; seg.querySelectorAll('.seg-b').forEach(function(x){x.classList.remove('on');}); t.classList.add('on'); };
    sv.onclick=async function(){
      ...
      const cc=seg?seg.querySelector('.seg-b.on'):null; const cat=cc?cc.getAttribute('data-v'):'런칭랜딩';
      const mo=(document.getElementById('apMemo').value||'').trim().slice(0,500);
      const msg=document.getElementById('apMsg'); msg.textContent='저장 중…';
      const pid='u'+Date.now(); const catv=(cat==='명소')?'spot':(cat==='candidate'?'candidate':'canoe');
      try{ const r=await fetch(fapi('/placeover'),{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({adminKey:adminKey(),id:pid,new:1,name:nm,memo:mo,cat:catv,lat:a.lat,lng:a.lng})});
        if(r.ok){ msg.textContent='완료!'; _placeOver[pid]={new:1,name:nm,cat:catv,lat:a.lat,lng:a.lng,memo:mo}; renderNewPlace(pid,_placeOver[pid]); gaEvent('place_add',{cat:cat}); setTimeout(function(){map.closePopup();},700); }
...
function _wpStaffHtml(s,p){
  if(!s){
    const noted=String((p&&p.safetyAction)||'').indexOf('안전관리요원')>=0;
    return '<div style="margin-top:7px;padding-top:6px;border-top:1px solid #e5e9ec"><b>안전관리요원</b><br>'
      +(noted?'<span style="font-weight:700;color:#e65100">배치 대상 기록 있음</span><br><span style="color:#778">'+pmEsc(String((p&&p.sourceYear)||2025))+'년 행안부 자료 · 현재 배치 기간·시간 미확인</span>'
...
function courseLineColor(sc, seed){
  const base = sc==='엑스페디션'?265 : sc==='초심자코스'?300 : 170;
...
const _ownedCourseTargets=new Set();
...
function _applyCourseOver(f){
  ...
  if(cid!=null && _courseOver[String(cid)]){
    ...
    if(validCourseColor(o.color)) p.color=validCourseColor(o.color);
    f.properties=p;
  }
  return f;
}
function _applyCourseFocus(){
  ...
  const showStaticAll = !isFocused && isAdmin();
  const showOwnedKv = !isFocused;
  Object.keys(_staticCidLayers).forEach(function(cid){
    const arr=_staticCidLayers[cid]; if(!arr) return;
    const keep=showStaticAll || (focusStatic && String(cid)===focusStatic);
  ...
  Object.keys(_kvCourseLayers).forEach(function(cid){
    ...
    const keep=showOwnedKv || (focusKv && String(cid)===focusKv);
...
function _renderStaticCourses(){
  ...
  COURSES.features.forEach(function(f){ _applyCourseOver(f); const p=f.properties||{};
    ...
    if(p.cid!=null) _courseByCid[String(p.cid)]={id:p.cid, static:true, name:p.name||'코스', km:p.km||0, color:validCourseColor(p.color), coords:(f.geometry&&f.geometry.coordinates||[]).map(function(c){return [c[1],c[0]];})};
...
fetch(courseReadUrl('/course?over=1')).then(function(r){return r.json();}).then(function(o){ Object.assign(_courseOver,o||{}); _renderStaticCourses(); }).catch(function(){ _renderStaticCourses(); });
...
function _weatherKey(lat,lng){ return lat.toFixed(2)+','+lng.toFixed(2); }
function _weatherFromJson(j){
  const cu=j&&j.current; if(!cu) return null;
...
function fetchWeatherData(lat,lng){
  const key=_weatherKey(lat,lng), c=_wxCache[key];
  if(c && Date.now()-c.ts<600000) return Promise.resolve(c.w);
  return fetch('https://api.open-meteo.com/v1/forecast?latitude='+lat+'&longitude='+lng
    ...
    .then(function(r){return r.json();})
    .then(function(j){ const w=_weatherFromJson(j); if(w) _wxCache[key]={w:w,ts:Date.now()}; return w; })
    .catch(function(){ return null; });
}
function _renderWeatherBox(el,w){
  const AR=['↓','↙','←','↖','↑','↗','→','↘'];   // 풍향(불어오는 방향) → 부는 쪽 화살표
  function render(one){
    if(!one){ el.innerHTML=''; return; }
    const wind=one.wind, col=_windCol(wind);
    let h='<div class="pm-wx-now"><span>'+_wxEmoji(one.code)+' '+one.temp.toFixed(0)+'° '+_wxText(one.code)+'</span>'
      +'<span style="color:'+col+';font-weight:'+(wind>=5?'700':'400')+'">바람 '+wind.toFixed(1)+'㎧ '+_wxDir(one.dir)+(wind>=8?' ⚠️강풍':wind>=5?' 주의':'')+'</span>'
      +'<span style="color:#1565c0">강수 '+((one.rain||0)<10?(one.rain||0):Math.round(one.rain))+'㎜</span>'+'</div>';
    h+='<div class="pm-wx-tabs"><a data-v="h" class="'+(_wxView==='h'?'on':'')+'">시간대별</a><a data-v="d" class="'+(_wxView==='d'?'on':'')+'">주간</a></div>';
    if(_wxView==='h' && one.hours && one.hours.length){
      h+='<div class="pm-wx-days pm-wx-hscroll">'+one.hours.map(function(d){
    ...
    } else if(one.days&&one.days.length){
      h+='<div class="pm-wx-days">'+one.days.map(function(d){
        const ar=AR[Math.round(((d.wdir%360)+360)%360/45)%8];
    ...
    el.innerHTML=h;
    el.querySelectorAll('.pm-wx-tabs a').forEach(function(a){ a.onclick=function(){ _wxView=a.getAttribute('data-v'); try{localStorage.setItem('mc_wxview',_wxView);}catch(e){} render(one); }; });
  }
  render(w);
}
function placeWeather(lat,lng){
  ...
  const key=_weatherKey(lat,lng), c=_wxCache[key];
  if(c && Date.now()-c.ts<600000){ _renderWeatherBox(el,c.w); return; }
  el.innerHTML='<span class="wl-loading">날씨 조회…</span>';
  fetchWeatherData(lat,lng).then(function(w){ _renderWeatherBox(el,w); }).catch(function(){ _renderWeatherBox(el,null); });
}
function courseWeather(lat,lng,elId){
  const el=document.getElementById(elId||'cmWx'); if(!el) return;
  const key=_weatherKey(lat,lng), c=_wxCache[key];
  if(c && Date.now()-c.ts<600000){ _renderWeatherBox(el,c.w); return; }
  el.innerHTML='<span class="wl-loading">날씨 조회…</span>';
  fetchWeatherData(lat,lng).then(function(w){ _renderWeatherBox(el,w); }).catch(function(){ _renderWeatherBox(el,null); });
...
function openCourseComments(key,course,shareId){
  ...
  const name=course.name||'코스', km=course.km||0;
  _pmPlace={name:name}; _pmSlug=key;
  ...
  _clearPhoto(); renderRateRow();
  const canAnalyze=!!(course.coords && course.coords.length>=2);
  const actions=[
    (shareId?'<a class="course-btn" onclick="shareCourse(\''+shareId+'\')">🔗 공유</a>':''),
  ...
  document.getElementById('pmLinks').innerHTML=(km?'<div class="pm-memo">약 '+km+'km</div>':'')
    +((actions||_cadm)?'<div class="course-actions">'+actions+'</div>':'')
    +_cadm
    +'<div id="courseAnalysisPanel" class="cm-analysis" style="display:none"></div>';
  document.getElementById('pmAdmin').innerHTML='';
  document.getElementById('pmCmts').innerHTML='<div class="pm-empty">불러오는 중…</div>';
  document.getElementById('pmCnt').textContent=''; document.getElementById('pmInput').value=''; document.getElementById('pmMsg').textContent='';
  document.getElementById('pmodal').classList.add('open');
...
async function toggleCourseAnalysis(){
  const box=document.getElementById('courseAnalysisPanel'); if(!box || !_courseInfoCourse || !_courseInfoCourse.coords || !_courseInfoCourse.coords.length) return;
  const on=box.style.display!=='none';
  if(on){ box.style.display='none'; return; }
  box.style.display='block';
  await renderCourseAnalysis(_courseInfoCourse,'courseAnalysisPanel');
}
...
function _obKakaoLink(o){
  const exact=_obKakaoUrl(o&&o.kakaoUrl); if(exact) return exact;
  const nm=String(o&&o.name||'식당/카페').replace(/,/g,' ').trim().slice(0,30)||'식당/카페';
  return 'https://map.kakao.com/link/map/'+encodeURIComponent(nm)+','+Number(o.lat)+','+Number(o.lng);
}
...
function obsPopup(o){ const t=OBS_TYPES[o.type]||OBS_TYPES['보']; const nm=(o.name&&String(o.name).trim())?pmEsc(String(o.name).trim()):'';
  ...
  return '<span class="obs-ic '+t.c+'">'+t.e+' '+(nm||t.label)+'</span>'+(nm?'<small style="color:#99a;margin-left:6px">'+t.label+'</small>':'')+(o.note?'<div style="margin:7px 0 4px;color:#445;font-size:13px">'+linkify(o.note)+'</div>':'<br>')+kakao
...
function renderStaticWeir(w){ if(!w||w.lat==null) return; renderObstacle({id:staticWeirId(w),lat:w.lat,lng:w.lng,type:'보',note:'',name:w.nm}); }
...
function obPick(el){ const bs=document.querySelectorAll('#obBody .seg-b'); for(let i=0;i<bs.length;i++) bs[i].classList.remove('on'); el.classList.add('on');
  const ty=el.getAttribute('data-ty'), row=document.getElementById('obNameRow'), kr=document.getElementById('obKakaoRow');
  if(row) row.style.display=_obHasName(ty)?'block':'none'; if(kr) kr.style.display=_obHasKakao(ty)?'block':'none'; }
function openObsModal(mode,data){ if(!isAdmin()) return;
  _obMode=mode; let ty='보', note='', name='', kakaoUrl='';
  if(mode==='edit'){ _obCur=data; _obLL={lat:data.lat,lng:data.lng}; ty=data.type; note=data.note||''; name=data.name||''; kakaoUrl=data.kakaoUrl||''; }
  ...
  document.getElementById('obBody').innerHTML=
    ...
    +'<div id="obKakaoRow" style="display:'+(_obHasKakao(ty)?'block':'none')+'"><div class="sg-label">카카오맵 장소 URL (선택)</div>'
    +'<input id="obKakaoUrl" maxlength="500" placeholder="카카오맵에서 공유한 장소 링크" value="'+pmEsc(kakaoUrl)+'"></div>'
...
async function doSaveObs(){ if(!isAdmin()||!_obLL) return;
  ...
  try{ let body;
    if(_obMode==='edit'&&_obCur){ body={action:'edit',adminKey:adminKey(),obsId:_obCur.id,type:type,note:note,name:name,kakaoUrl:kakaoUrl}; }
    else { body={action:'add',adminKey:adminKey(),lat:_obLL.lat,lng:_obLL.lng,type:type,note:note,name:name,kakaoUrl:kakaoUrl}; }
    ...
    if(r.ok){ const d=await r.json().catch(function(){return {};}); gaEvent('obstacle_'+_obMode); closeObsModal();
      if(_obMode==='edit'&&_obCur){ if(_obCur._m) obstacleLayer.removeLayer(_obCur._m); renderObstacle(Object.assign({},_obCur,{type:type,note:note,name:name,kakaoUrl:kakaoUrl,_m:null})); }
      else { renderObstacle((d&&d.obstacle)||{id:Date.now(),lat:_obLL.lat,lng:_obLL.lng,type:type,note:note,name:name,kakaoUrl:kakaoUrl}); }
      map.closePopup();
...
function canoeIcon(kind){ kind=kind||'canoe'; return L.divIcon({className:'canoe-pin'+(kind==='candidate'?' candidate':''),html:'<span class="canoe-pin-in'+(kind==='candidate'?' candidate':'')+'">'+canoeSvg(kind)+'</span>',iconSize:[26,26],iconAnchor:[13,13]}); }
...
const BEAVERTAIL_PADDLE_SVG='<svg viewBox="0 0 32 32" aria-hidden="true"><g transform="rotate(-34 16 16)"><path d="M15.05 4.2h1.9c.65 0 1.18.53 1.18 1.18v13.1c0 .65-.53 1.18-1.18 1.18h-1.9c-.65 0-1.18-.53-1.18-1.18V5.38c0-.65.53-1.18 1.18-1.18Z" fill="#8d5a2b"/><path d="M16 1.75c1.95 0 3.85 1.06 3.85 2.42 0 .62-.38 1.16-.99 1.55-.78.5-1.8.75-2.86.75s-2.08-.25-2.86-.75c-.61-.39-.99-.93-.99-1.55 0-1.36 1.9-2.42 3.85-2.42Z" fill="#8d5a2b"/><path d="M16 18.35c-4.15 0-7.25 2.18-7.25 5.88 0 2.34 1.22 4.78 3.2 5.86 1.17.64 2.62.96 4.05.96s2.88-.32 4.05-.96c1.98-1.08 3.2-3.52 3.2-5.86 0-3.7-3.1-5.88-7.25-5.88Z" fill="#f7f1df"/><path d="M16 18.35c-3.1 0-5.55 1.74-5.55 4.84 0 2 .87 4.08 2.35 5 .92.57 2.06.86 3.2.86s2.28-.29 3.2-.86c1.48-.92 2.35-3 2.35-5 0-3.1-2.45-4.84-5.55-4.84Z" fill="#c77b32"/><path d="M13.3 19.25c.55 2.9.48 6.02-.2 8.86M16 18.75v10.45M18.7 19.25c-.55 2.9-.48 6.02.2 8.86" fill="none" stroke="#e2ad6b" stroke-width=".75" stroke-linecap="round" opacity=".9"/></g></svg>';
function beavertailPaddleIcon(){ return L.divIcon({className:'spot-pin',html:'<span class="spot-pin-in">'+BEAVERTAIL_PADDLE_SVG+'</span>',iconSize:[30,30],iconAnchor:[15,15]}); }
...
function _bindPlaceClick(m, getPlace, label){
...
function renderNewPlace(id,o){ if(_placeMarkerById[id]||o.del||o.lat==null) return;
  ...
  const m=makeMarker([o.lat,o.lng],k); _bindPlaceClick(m,function(){return rec;},function(){return rec.name||'장소';}); m.addTo(k==='spot'?famousLayer:canoeLayer);
...
function courseReadUrl(path){
  let v=''; try{ v=localStorage.getItem('mc_course_v')||''; }catch(e){}
  return fapi(path)+(v?(path.indexOf('?')>=0?'&':'?')+'_v='+encodeURIComponent(v):'');
}
function bumpCourseVersion(){
  try{ localStorage.setItem('mc_course_v',String(Date.now())); }catch(e){}
}
...
function applyFavFilter(){
  Object.keys(_favLayerIdx).forEach(function(t){
    const keep=!_favOnly || _favSet.has(t) || _ownedCourseTargets.has(t);
...
function renderPmCat(pl){   // 관리자 전용: 장소 분류(명소/런칭·랜딩) 토글
  ...
  el.innerHTML='<span class="pm-catlbl">분류(관리자)</span>'
    ...
    +'<a data-k="candidate" class="pm-cat'+(cur==='candidate'?' on':'')+'">🟠 후보지</a>';
...
function openMyPage(){ const u=getUser(); if(!u||!u.uid) return;
  ...
  async function loadMyCourses(){
    try{
      const r=await fetch(courseReadUrl('/courses?mine=1&uid='+encodeURIComponent(u.uid)+'&tok='+encodeURIComponent(u.tok||'')));
      return await r.json();
    }catch(e){ return []; }
  }
...
function _wlGetCode(cd){
  const direct = HRFCO_KEY
    ? fetch('https://api.hrfco.go.kr/'+HRFCO_KEY+'/waterlevel/list/10M/'+cd+'.json')
        .then(function(r){return r.json();})
        // 일부 중단 관측소는 결측을 0.00으로 반환한다. 0은 시간·일자료에서 다시 확인한 뒤 채택한다.
        .then(function(j){ const rec=(j.content||[])[0],wl=rec&&parseFloat(rec.wl); return (rec&&isFinite(wl)&&wl!==0)?{wl:rec.wl,fw:rec.fw,t:rec.ymdhm,src:'10M',cd:cd}:null; })
  ...
  return direct.then(function(rec){ return rec || _wlRecent1H(cd); }).catch(function(){   // 직접 호출 실패 시 워커 폴백
      return fetch(WORKER_URL.replace(/\/+$/,'')+'/waterlevel?obs='+cd)
        .then(function(r){return r.json();}).then(function(d){ const rec=(d&&d[cd])||null,wl=rec&&parseFloat(rec.wl); return (rec&&isFinite(wl)&&wl!==0)?{wl:rec.wl,fw:rec.fw,t:rec.t,src:'10M',cd:cd}:null; });
...
function _wlSpark24M(s,cd){
  ...
  return get(from,midPrev).catch(function(){return [];}).then(function(oldRows){
    return get(mid,now).catch(function(){return [];}).then(function(newRows){return [oldRows,newRows];});
  }).then(function(parts){const byDay={};parts[0].concat(parts[1]).forEach(function(r){byDay[ymd(r.t)]=r;});const rows=Object.keys(byDay).map(function(k){return byDay[k];}).sort(function(a,b){return a.t-b.t;});_wlSparkCache[cd]={rows:rows,ts:Date.now()};return build(rows);}).catch(function(){return '';});
}
function _wlYear(cd, cur){   // 지난 1년(1D) 일자료 분포에서 현재 수위 백분위 — 게이지 + 범위
  ...
  const build=function(vals){
    ...
    const fromMin=Math.max(0,cur-mn), toMax=Math.max(0,mx-cur);
  ...
  m.on('click',function(){
    _wlGet(s).then(function(rec){
      ...
      if(rec&&rec.wl&&isFinite(parseFloat(rec.wl))){ const st=_wlStage(rec.wl,s);
        ...
        h+='<div id="wlTrend_'+s.cd+'"></div><div id="wlSpark_'+s.cd+'"><small style="color:#889">24개월 수위 불러오는 중…</small></div><div id="wlYr_'+s.cd+'"></div>';
      ...
      if(rec){_wlSpark24M(s,qcd).then(function(html){const el=document.getElementById('wlSpark_'+s.cd);if(el)el.innerHTML=html||'<small style="color:#999">24개월 수위 자료 없음</small>';});_wlYear(qcd, parseFloat(rec.wl)).then(function(html){ const el=document.getElementById('wlYr_'+s.cd); if(el&&html) el.innerHTML=html; });}
...
const _damCache={}, _damYrCache={}, _damSparkCache={}, _damDayCache={};
...
function _damSpark24H(d){
  ...
  const build=function(rows){
    ...
    const mn=Math.min.apply(null,vals), mx=Math.max.apply(null,vals), rng=(mx-mn)||0.01;
    const W=210,H=38,padL=3,padR=3,padT=5,padB=10,plotW=W-padL-padR,plotH=H-padT-padB;
    const first=rows[0].t.getTime(), last=rows[rows.length-1].t.getTime(), span=(last-first)||1;
    ...
    const dlt=vals[vals.length-1]-vals[0];
    const arrow=Math.abs(dlt)<0.02?'─ 보합':(dlt>0?'<span style="color:#e53935">▲ +'+dlt.toFixed(2)+'m</span>':'<span style="color:#1565c0">▼ '+dlt.toFixed(2)+'m</span>');
    return '<div style="margin-top:8px"><small style="color:#667;font-weight:700">지난 24시간 저수위</small>'
      ...
      +'<polyline points="'+pts+'" fill="none" stroke="#00acc1" stroke-width="1.8"/>'
      +'<circle cx="'+xOf(rows[rows.length-1].t).toFixed(1)+'" cy="'+yOf(vals[vals.length-1]).toFixed(1)+'" r="2.2" fill="#263238"/>'
      +'</svg><small style="color:#889">24시간 변화 '+arrow+' · 범위 '+mn.toFixed(2)+'~'+mx.toFixed(2)+'m</small></div>';
  };
  const c=_damDayCache[d.cd];
  if(c && Date.now()-c.ts<600000) return Promise.resolve(build(c.rows));
  function parseHour(s){ s=String(s||''); if(s.length<10) return null; return new Date(+s.slice(0,4),+s.slice(4,6)-1,+s.slice(6,8),+s.slice(8,10)); }
  const now=new Date(), from=new Date(now.getTime()-24*3600*1000);
  return fetch('https://api.hrfco.go.kr/'+HRFCO_KEY+'/dam/list/1H/'+d.cd+'/'+_hrfcoYmdh(from)+'/'+_hrfcoYmdh(now)+'.json')
    .then(function(r){return r.json();})
    .then(function(j){ const rows=(j.content||[]).map(function(x){return {t:parseHour(x.ymdhm),v:parseFloat(x.swl)};}).filter(function(x){return x.t&&isFinite(x.v)&&x.v>0;});
      _damDayCache[d.cd]={rows:rows,ts:Date.now()}; return build(rows); })
    .catch(function(){ return ''; });
}
function _damSpark24M(d){
  ...
  const build=function(rows){
    ...
    const vals=rows.map(function(x){return x.v;});
    const mn=Math.min.apply(null,vals), mx=Math.max.apply(null,vals);
    const fld=parseFloat(d.fld),pfh=parseFloat(d.pfh),ref=isFinite(fld)?fld:(isFinite(pfh)?pfh:NaN),refName=isFinite(fld)?'제한수위':(isFinite(pfh)?'계획홍수위':'');
    const axisMin=mn,axisMax=Math.max(mx,isFinite(ref)?ref:mx),rng=(axisMax-axisMin)||0.01;
    const W=210,H=54,padL=3,padR=3,padT=5,padB=15,plotW=W-padL-padR,plotH=H-padT-padB;
    const first=rows[0].t.getTime(), last=rows[rows.length-1].t.getTime(), span=(last-first)||1;
    const xOf=function(t){ return padL+(t.getTime()-first)/span*plotW; };
    const yOf=function(v){ return padT+plotH-(v-axisMin)/rng*plotH; };
    const pts=rows.map(function(r){ return xOf(r.t).toFixed(1)+','+yOf(r.v).toFixed(1); }).join(' ');
    let ticks='', labs='';
    const cursor=new Date(rows[0].t.getFullYear(), rows[0].t.getMonth(), 1);
    ...
    let refLine='';
    if(isFinite(ref)&&ref>=axisMin&&ref<=axisMax){const ry=yOf(ref);refLine='<line x1="'+padL+'" y1="'+ry.toFixed(1)+'" x2="'+(W-padR)+'" y2="'+ry.toFixed(1)+'" stroke="#c62828" stroke-width="1" stroke-dasharray="3 3"/><text x="'+(W-padR-1)+'" y="'+Math.min(H-padB-1,ry+7).toFixed(1)+'" text-anchor="end" font-size="7" fill="#b71c1c">'+refName+'</text>';}
    return '<div style="margin-top:8px"><small style="color:#667;font-weight:700">지난 '+months+'개월 저수위 추이</small>'
      +'<svg width="'+W+'" height="'+H+'" viewBox="0 0 '+W+' '+H+'" style="display:block;margin-top:2px;max-width:100%">'
      +'<rect x="0" y="0" width="'+W+'" height="'+H+'" rx="6" fill="#f7fafb"/>'
      +ticks+refLine+'<polyline points="'+pts+'" fill="none" stroke="#1976d2" stroke-width="1.7"/>'
      +'<circle cx="'+xOf(rows[rows.length-1].t).toFixed(1)+'" cy="'+yOf(cur).toFixed(1)+'" r="2.3" fill="#263238"/>'
      +labs+'</svg>'
      +'<small style="color:#889;display:block;margin-top:1px">'+period+' · 실제 범위 '+mn.toFixed(2)+'~'+mx.toFixed(2)+'m · 변화 '+dTxt+(isFinite(ref)?' · 축 상단 '+refName+' '+ref.toFixed(2)+'m':'')+'</small></div>';
  };
  ...
  m.on('click',function(){
    _damGet(d).then(function(rec){
      ...
      if(rec){
        ...
        _damSpark24H(d).then(function(html){ const el=document.getElementById('damDay_'+d.cd); if(el&&html) el.innerHTML=html; });
...
const _riverSearchFocus=L.layerGroup().addTo(map);
const _riverSearchPane=map.createPane('riverSearchPane');_riverSearchPane.style.zIndex='480';_riverSearchPane.style.pointerEvents='none';
let _riverMain=null,_riverStream=null,_riverLoading=null,_riverControlAdded=false,_riverFeatures=[],_riverPopup=null,_riverPopupSeq=0,_riverFocusCtl=null;
function _showRiverFocusBar(name){
  if(!_riverFocusCtl){
    const C=L.Control.extend({options:{position:'topleft'},onAdd:function(){const d=L.DomUtil.create('div','river-focus-bar');d.id='riverFocusBar';d.innerHTML='<span aria-hidden="true">🌊</span><span class="river-focus-name"></span><button class="river-focus-x" type="button" title="물줄기 표시 해제" aria-label="물줄기 표시 해제">×</button>';L.DomEvent.disableClickPropagation(d);L.DomEvent.disableScrollPropagation(d);d.querySelector('button').onclick=function(){clearRiverSearch();};return d;}});_riverFocusCtl=new C();map.addControl(_riverFocusCtl);
  }
  const d=document.getElementById('riverFocusBar');if(!d)return;d.querySelector('.river-focus-name').textContent=name||'선택한 물줄기';d.classList.add('on');
}
...
function _riverSearch(q){
  if(!isAdmin()||!_riverFeatures.length)return [];
  const nq=String(q||'').replace(/\s/g,'').toLowerCase(), groups={};
  _riverFeatures.forEach(function(f){const p=f.properties||{},n=String(p.name||'');if(!n||n.replace(/\s/g,'').toLowerCase().indexOf(nq)<0)return;(groups[n]||(groups[n]=[])).push(f);});
  const names=Object.keys(groups).sort(function(a,b){const ae=a.replace(/\s/g,'').toLowerCase()===nq,be=b.replace(/\s/g,'').toLowerCase()===nq;return ae===be?a.localeCompare(b,'ko'):(ae?-1:1);}),out=[];
  names.forEach(function(n){_riverComponents(groups[n],n).forEach(function(fs){if(out.length>=10)return;const f=fs[0],cs=f.geometry.coordinates,m=cs[Math.floor(cs.length/2)],region=_riverRegionLabel(m[1],m[0]);out.push({kind:'river',label:'🌊 '+n,sub:region+' · '+((f.properties||{}).kind==='river'?'강·하천':'지류 하천')+' · 전체 물줄기',lat:m[1],lng:m[0],name:n,features:fs});});});return out;
}
function _riverFeatureBox(f){const cs=(f&&f.geometry&&f.geometry.coordinates)||[];let s=90,w=180,n=-90,e=-180;cs.forEach(function(c){if(!c||!isFinite(c[0])||!isFinite(c[1]))return;w=Math.min(w,+c[0]);e=Math.max(e,+c[0]);s=Math.min(s,+c[1]);n=Math.max(n,+c[1]);});return {s:s,w:w,n:n,e:e};}
function _riverBoxGapKm(a,b){const dy=Math.max(0,a.s-b.n,b.s-a.n)*110.54,lat=(Math.max(a.s,b.s)+Math.min(a.n,b.n))/2,dx=Math.max(0,a.w-b.e,b.w-a.e)*111.32*Math.cos(lat*Math.PI/180);return Math.sqrt(dx*dx+dy*dy);}
function _riverFeaturesNear(a,b,limitKm){
  if(_riverBoxGapKm(_riverFeatureBox(a),_riverFeatureBox(b))>limitKm)return false;
  const ac=(a.geometry||{}).coordinates||[],bc=(b.geometry||{}).coordinates||[],as=Math.max(1,Math.floor(ac.length/120)),bs=Math.max(1,Math.floor(bc.length/120));
  for(let i=0;i<ac.length;i+=as)for(let j=0;j<bc.length;j+=bs)if(_haversineKm([ac[i][1],ac[i][0]],[bc[j][1],bc[j][0]])<=limitKm)return true;
  const ae=ac[ac.length-1],be=bc[bc.length-1];return !!(ae&&be&&([ac[0],ae].some(function(x){return [bc[0],be].some(function(y){return _haversineKm([x[1],x[0]],[y[1],y[0]])<=limitKm;});})));
}
function _riverComponents(features,name){
  const fs=(features||[]).slice(),parent=fs.map(function(_,i){return i;}),limit=/강$/.test(String(name||''))?12:2;
  ...
  for(let i=0;i<fs.length;i++)for(let j=i+1;j<fs.length;j++)if(_riverFeaturesNear(fs[i],fs[j],limit))join(i,j);
  const out={};fs.forEach(function(f,i){const k=root(i);(out[k]||(out[k]=[])).push(f);});
  return Object.keys(out).map(function(k){return out[k].sort(function(a,b){return b.geometry.coordinates.length-a.geometry.coordinates.length;});}).sort(function(a,b){return b.reduce(function(s,f){return s+f.geometry.coordinates.length;},0)-a.reduce(function(s,f){return s+f.geometry.coordinates.length;},0);});
}
const _riverRegionCache={};
function _riverAdminName(addr){
  const s=((addr&&addr.parcel)|| (addr&&addr.road)||''),matches=s.match(/[가-힣]+(?:시|군)/g)||[];
  return matches.filter(function(x){return !/(특별시|광역시|자치시)$/.test(x);}).pop()||matches.pop()||'';
}
async function _enrichRiverRegions(rows){
  await Promise.all((rows||[]).map(async function(x){
    const f=x.features&&x.features[0],cs=f&&f.geometry&&f.geometry.coordinates;if(!cs||!cs.length)return;
    const key=x.name+'|'+x.lat.toFixed(3)+','+x.lng.toFixed(3);if(_riverRegionCache[key]){x.region=_riverRegionCache[key];x.sub=x.region+' · '+((f.properties||{}).kind==='river'?'강·하천':'지류 하천')+' · 전체 물줄기';return;}
    const idx=[Math.floor((cs.length-1)*.12),Math.floor((cs.length-1)*.5),Math.floor((cs.length-1)*.88)],names=[];
    const found=await Promise.all(idx.map(function(i){const p=cs[i];return vworldReverse(p[1],p[0]).then(_riverAdminName).catch(function(){return '';});}));
    found.forEach(function(n){if(n&&names.indexOf(n)<0)names.push(n);});
    const region=names.join(' · ')||_riverRegionLabel(x.lat,x.lng);_riverRegionCache[key]=region;x.region=region;x.sub=region+' · '+((f.properties||{}).kind==='river'?'강·하천':'지류 하천')+' · 전체 물줄기';
  }));return rows;
}
function _riverRegionLabel(lat,lng){
  if(lat>=37.50&&lat<=37.72&&lng>=126.76&&lng<=127.20)return '서울';
  if(lat>=37.30&&lat<37.50&&lng>=126.80&&lng<=127.18)return '안양/과천';
  if(lat>=37.0&&lng>=126.55&&lng<=127.85)return '경기';
  if(lat>=37.0&&lng>127.65)return '강원';
  if(lat<35.8&&lng>=127.55&&lng<=129.6)return '경남';
  if(lat<36.4&&lng<127.55)return '전라';
  if(lat<37.2&&lng>=127.5)return '충청/경북';
  return '대한민국';
}
...
function focusRiverSearch(x,keepUrl){
  ...
  (x.features||[]).forEach(function(f){const ll=f.geometry.coordinates.map(function(c){return [c[1],c[0]];});if(ll.length<2)return;if(!anchor)anchor=ll[Math.floor(ll.length/2)];L.polyline(ll,{pane:'riverSearchPane',color:'#001a33',weight:8,opacity:.68,interactive:false}).addTo(_riverSearchFocus);L.polyline(ll,{pane:'riverSearchPane',color:'#003b73',weight:4.5,opacity:1,interactive:false}).addTo(_riverSearchFocus);bounds.extend(ll);});
  if(bounds.isValid()&&anchor){map.fitBounds(bounds.pad(.06),{maxZoom:13,animate:false});const c=L.latLng(anchor),enc=encodeURIComponent(x.name);_riverPopup=L.popup({minWidth:190,autoPan:true,closeButton:true}).setLatLng(c).setContent('<b>🌊 '+pmEsc(x.name)+'</b><br><small>'+pmEsc(_riverRegionLabel(c.lat,c.lng))+' · 전체 물줄기 강조 표시</small><br><button class="addplace-btn" onclick="shareRiverEncoded(\''+enc+'\','+c.lat+','+c.lng+')">공유</button> <button class="addplace-btn" onclick="clearRiverSearch()">강조 해제</button>');setTimeout(function(){if(seq===_riverPopupSeq&&_riverPopup)_riverPopup.openOn(map);},80);}gaEvent('river_search',{name:x.name});
}
function focusRiverFromUrl(){
  const u=new URL(location.href),name=u.searchParams.get('river'),at=(u.searchParams.get('riverAt')||'').split(',').map(Number);if(!name)return;
  _loadAdminRivers().then(function(){const fs=_riverFeatures.filter(function(f){return String((f.properties||{}).name||'')===name;});if(!fs.length)return;let parts=_riverComponents(fs,name),chosen=parts[0];if(isFinite(at[0])&&isFinite(at[1])){function d(part){let best=Infinity;part.forEach(function(f){const cs=f.geometry.coordinates,step=Math.max(1,Math.floor(cs.length/150));for(let i=0;i<cs.length;i+=step)best=Math.min(best,_haversineKm(at,[cs[i][1],cs[i][0]]));});return best;}parts.sort(function(a,b){return d(a)-d(b);});chosen=parts[0];}focusRiverSearch({name:name,features:chosen},true);});
}
...
(function(){ const c=_layerControl.getContainer(); if(!c) return;
  const h=L.DomUtil.create('div','lc-title'); h.innerHTML='<span>🗺️ 레이어 · 범례</span><span class="lc-arrow">▾</span>';h.setAttribute('role','button');h.setAttribute('tabindex','0');h.setAttribute('aria-expanded','true');c.insertBefore(h, c.firstChild);
  const k=L.DomUtil.create('div','lc-key');
  k.innerHTML='<div class="lg-sub">🛟 물놀이 관리지역</div>'
  ...
  if(isTouch){c.classList.add('lc-collapsed');h.setAttribute('aria-expanded','false');}   // 모바일: 기본 닫힘
})();
...
(function(){
  ...
  p.innerHTML='<div class="wayback-head"><span>과거 위성 · <span id="waybackName"></span></span><button id="waybackClose" type="button" title="과거 영상 닫기">×</button></div>'
    ...
    +'<small class="wayback-note">선택한 댐 위치의 영상이 실제로 바뀐 배포본만 표시합니다. 왼쪽은 현재, 오른쪽은 과거 영상이며 실제 촬영일은 지역별로 다를 수 있습니다.</small>';
...
function renderKVCourse(c){
  ...
  _kvCourses[c.id]=c;
  if(_isCourseOwner(c)||(isAdmin()&&String(c.owner||'')==='admin'))_ownedCourseTargets.add('course_k'+c.id);
...
function _clearKVCourses(){Object.keys(_kvCourseLayers).forEach(function(id){const e=_kvCourseLayers[id];if(e)e.ls.forEach(function(l){e.grp.removeLayer(l);});_ownedCourseTargets.delete('course_k'+id);delete _kvCourseLayers[id];delete _kvCourses[id];});}
...
// 정적 코스 숨김(관리자 삭제분) 적용
fetch(courseReadUrl('/course?hidden=1')).then(function(r){return r.json();}).then(function(a){ (a||[]).forEach(function(cid){ _hiddenStaticCids.add(String(cid)); _hideStaticCourse(cid); }); }).catch(function(){});
loadCourses();
// ---- 코스 URL 공유 ----
...
function _findSharedKvCourse(list,kid){
  const exact=(list||[]).find(function(x){return String(x.id)===String(kid);}); if(exact) return exact;
  const n=Number(kid); if(!isFinite(n)) return null;
  let best=null, gap=Infinity; (list||[]).forEach(function(x){ const d=Math.abs(Number(x.id)-n); if(d<gap){gap=d;best=x;} });
  return gap<=5000?best:null;   // 구버전은 등록 직후 브라우저/서버 ID가 수 ms 어긋날 수 있음
}
...
const MeasureCtl = L.Control.extend({ options:{position:'topleft'},
  onAdd:function(){
    const d=L.DomUtil.create('div','measbtn'); d.id='measBtnBox'; d.innerHTML='📏 코스만들기<br><span class="meas-sub">(거리측정)</span>';
    L.DomEvent.disableClickPropagation(d); L.DomEvent.disableScrollPropagation(d);
...
function updateMeasBtn(){
  ...
  if(!measureMode){ b.innerHTML='📏 코스만들기<br><span class="meas-sub">(거리측정)</span>'; return; }
...
function _drawMeasAccess(grp, access){
  (access||[]).forEach(function(a){
    if(!a||a.length<2||map.distance(a[0],a[1])<12) return;
    L.polyline(a,{color:'#607d8b',weight:2.5,opacity:.8,dashArray:'3,6',interactive:false}).addTo(grp);
  });
}
...
async function _runMeasQueue(){
  ...
  while(_measQueue.length){
    ...
    if(!seg || seg.err){
      const why=seg&&seg.err==='overpass'?'서버 혼잡 — 잠시 후 다시 클릭':seg&&seg.err==='farwater'?'가까운 물길이 없습니다<br><small>강안에 더 가까운 지점을 선택하세요</small>':seg&&seg.err==='detour'?'비정상적으로 우회하는 물길입니다<br><small>경유지를 추가해 구간을 나눠주세요</small>':'이 구간 물길 못 찾음<br><small>더 가까운 점/경유지를 찍어보세요</small>';
      L.popup({closeButton:false}).setLatLng(pt).setContent(why).openOn(map);
      _redrawMeasDraft();
...
function finishMeasure(){
  ...
  if(isAdmin() || _curUid()){   // 로그인 사용자도 이 경로를 자기 코스로 등록
    const riverCounts={};measSegs.forEach(function(sg){if(sg.riverName)riverCounts[sg.riverName]=(riverCounts[sg.riverName]||0)+1;});
    const riverName=Object.keys(riverCounts).sort(function(a,b){return riverCounts[b]-riverCounts[a];})[0]||'';
    const first=measPts[0],lastPt=measPts[measPts.length-1];
    _lastCourse={coords:cc, km:Math.round(km*100)/100, riverName:riverName,
      start:{lat:first.lat,lng:first.lng,label:first._measLabel||''},end:{lat:lastPt.lat,lng:lastPt.lng,label:lastPt._measLabel||''},
...
function _haversineKm(a,b){
  ...
  const R=6371;
  const dLat=_deg2rad((b[0]-a[0]||0));
  const dLng=_deg2rad((b[1]-a[1]||0));
  const la1=_deg2rad(a[0]), la2=_deg2rad(b[0]);
  const h=Math.sin(dLat/2)*Math.sin(dLat/2)+Math.cos(la1)*Math.cos(la2)*Math.sin(dLng/2)*Math.sin(dLng/2);
  return 2*R*Math.asin(Math.sqrt(h));
}
function _courseCenter(coords){
  if(!coords||!coords.length) return null;
  let lat=0,lng=0,n=0;
  coords.forEach(function(p){ if(p&&isFinite(p[0])&&isFinite(p[1])){ lat+=+p[0]; lng+=+p[1]; n++; } });
  return n?{lat:lat/n,lng:lng/n}:null;
}
function _coursePointMinKm(pt, coords){
  let best=Infinity;
  coords.forEach(function(p){ const d=_haversineKm(pt,p); if(d<best) best=d; });
  return best;
}
function _courseStationRank(coords, limit){
  const list=(WLSTN||[]).map(function(s){
    const pt=[+s.lat,+s.lng];
    let d=_coursePointMinKm(pt, coords);
    return {s:s, d:d};
  }).filter(function(x){ return isFinite(x.d); }).sort(function(a,b){ return a.d-b.d; });
  const seen=new Set(), out=[];
  list.forEach(function(x){ if(out.length>=limit) return; if(seen.has(String(x.s.cd))) return; seen.add(String(x.s.cd)); out.push(x); });
  return out;
}
function _stageRank(label){ return label==='심각'?4:label==='경계'?3:label==='주의'?2:label==='관심'?1:0; }
function _trendClass(d){ return d>0.03?'up':(d<-0.03?'down':'flat'); }
function _fmtDelta(d){ if(!isFinite(d)) return '변화 없음'; if(Math.abs(d)<0.01) return '보합'; return (d>0?'▲ +':'▼ ')+Math.abs(d).toFixed(2)+'m/24h'; }
function _fmtMm(v){ return isFinite(v)?(v<10?v.toFixed(1):Math.round(v)):'-'; }
function _wlRecentTrend(cd){
  if(!HRFCO_KEY) return Promise.resolve(null);
  const now=new Date(), from=new Date(now.getTime()-24*3600*1000);
  return fetch('https://api.hrfco.go.kr/'+HRFCO_KEY+'/waterlevel/list/1H/'+cd+'/'+_hrfcoYmdh(from)+'/'+_hrfcoYmdh(now)+'.json')
    .then(function(r){return r.json();})
    .then(function(j){
      const vals=(j.content||[]).map(function(x){return parseFloat(x.wl);}).filter(isFinite).reverse();
      if(vals.length<2) return null;
      const first=vals[0], last=vals[vals.length-1];
      return {start:first,end:last,delta:last-first,min:Math.min.apply(null,vals),max:Math.max.apply(null,vals)};
    })
    .catch(function(){ return null; });
}
function _courseVerdict(risk,wx){
  if(risk>=3) return {label:'비추천', cls:'cm-ana-bad', note:'경보 수위 또는 그에 준하는 상태입니다.'};
  if(risk===2 || (wx&&(wx.rain24>=15 || wx.wind24>=8))) return {label:'주의', cls:'', note:'수위 또는 날씨 조건을 함께 봐야 합니다.'};
  return {label:'적합', cls:'', note:'현재 기준으로는 큰 문제 없어 보입니다.'};
}
function _dayRisk(wxDay){
  if(!wxDay) return 0;
  if((+wxDay.rain||0)>=30 || (+wxDay.wmax||0)>=12) return 2;
  if((+wxDay.rain||0)>=15 || (+wxDay.wmax||0)>=8) return 1;
  return 0;
}
function _riskLabel(v){ return v>=2?'위험':(v===1?'⚠️ 주의':'안정'); }
async function renderCourseAnalysis(course,targetId){
  const box=document.getElementById(targetId||'cmAnalysis');
  if(!box) return;
  ...
  const wlPs=stations.map(function(x){ return Promise.all([_wlGet(x.s), _wlRecentTrend(x.s.cd)]).then(function(arr){
    return {station:x.s, dist:x.d, rec:arr[0], tr:arr[1]};
  }); });
  const [wx, rows]=await Promise.all([wxP, Promise.all(wlPs)]);
  if(seq!==_courseAnalysisSeq) return;
  let risk=0, wxRisk=0, dailyCards='';
  if(wx){
    const rain24=wx.hours.slice(0,8).reduce(function(s,h){ return s+(+h.rain||0); },0);
    const wind24=Math.max.apply(null, wx.hours.slice(0,8).map(function(h){ return +h.wind||0; }).concat([0]));
    wx.rain24=rain24; wx.wind24=wind24;
    ...
    dailyCards=wx.days.slice(0,7).map(function(d){
      ...
      return '<div class="cm-ana-card cm-day-card"><div class="cm-ana-title">'+pmEsc(d.label||'')+'</div>'
        ...
        +'<div class="cm-ana-line"><b>'+lbl+'</b> · '+(dr===2?'강수/풍속이 높은 편':dr===1?'비·바람 영향 가능':'현재 조건상 안정적')+'</div></div>';
    }).join('');
  }
  const cards=rows.map(function(x){
    const s=x.station, rec=x.rec||{}, tr=x.tr||null;
    ...
    return '<div class="cm-ana-card cm-station-card"><div class="cm-ana-title">'+pmEsc(s.nm||'수위관측소')+' · '+x.dist.toFixed(1)+'km</div>'
      ...
      +'</div>'
      +(tr&&isFinite(tr.delta)?'<div class="cm-ana-line" style="color:#778">24시간 범위 '+tr.min.toFixed(2)+'~'+tr.max.toFixed(2)+'m</div>':'')
      +'</div>';
  }).join('');
  const verdict=_courseVerdict(Math.max(risk,wxRisk), wx);
  ...
  box.innerHTML='<div class="cm-analysis-h"><span>코스 분석 <span class="beta-tag">BETA</span></span><button type="button" class="cm-analysis-close" onclick="toggleCourseAnalysis()" title="닫기">×</button></div>'
    ...
    +'<div class="cm-row-hint"><span>주변 수위관측소</span><span>← 좌우로 스크롤 →</span></div><div class="cm-row">'+cards+'</div>';
}
...
function _cmColor(){ const e=document.querySelector('#cmPalette .cm-color.on'); return e?validCourseColor(e.getAttribute('data-color')):''; }
...
function _shortCoursePlace(label,addr){
  label=(label||'').trim();
  if(label){const halves=label.split(/\s+-\s+/),region=(halves[0]||'').replace(/\([^)]*\)/g,'').trim(),place=(halves.length>1?halves.slice(1).join(' - '):label).replace(/^(춘천|양구|곡성|구례|연천|홍천|인제|화천)\s*[()]/,'').trim();return (region&&place.indexOf(region)!==0?region+' ':'')+place;}
  const parts=(addr||'').trim().split(/\s+/).filter(Boolean);let region='',place='';
  for(const p of parts){if(!region&&/[시군구]$/.test(p)&&!/특별시$|광역시$/.test(p))region=p.replace(/[시군구]$/,'');if(/[리가동읍면]$/.test(p))place=p;}
  return [region,place].filter(Boolean).join(' ')||parts.slice(-2).join(' ');
}
async function _coursePointName(x){
  if(!x)return '';
  if(x.label)return _shortCoursePlace(x.label,'');
  let r=null;try{r=await vworldReverse(x.lat,x.lng);}catch(e){}
  let addr=(r&&(r.parcel||r.road))||'';if(!addr)try{addr=await nominatimReverse(x.lat,x.lng);}catch(e){}
  return _shortCoursePlace('',addr);
}
async function _suggestCourseName(){
  const c=_lastCourse;if(!c||!c.start||!c.end)return;
  const names=await Promise.all([_coursePointName(c.start),_coursePointName(c.end)]);if(_cmMode!=='add'||_lastCourse!==c||_cmNameTouched)return;
  const input=document.getElementById('cmName');if(!input||input.value.trim())return;
  const ends=[names[0]||'출발지',names[1]||'도착지'].join(' ~ '),suggested=(c.riverName?c.riverName+' - ':'')+ends;
  input.value=suggested;cmPreview();
}
function openCourseModal(mode, course){
  ...
  let cat='초심자코스', no='', desc='', km=0, npts=0, color='';
  if((mode==='edit'||mode==='editstatic')&&course){ const s=_splitCourse(course.name); cat=s.cat; no=s.no; desc=s.desc; km=course.km||0; npts=(course.coords||[]).length; color=validCourseColor(course.color); }
  ...
  document.getElementById('cmBody').innerHTML=
    ...
    +'<div class="sg-label">코스 색상</div><div class="cm-palette" id="cmPalette">'+palette+'</div>'
...
async function doSaveCourse(){
  ...
  const cat=_cmCat(), no=_cmNo(), color=_cmColor()||subcatColor(cat);
  ...
  try{
    let body;
    if(_cmMode==='editstatic'&&_cmCourse){ body={action:'editstatic',adminKey:adminKey(),cid:String(_cmCourse.id),name:name,km:_cmCourse.km,color:color}; }
    else if(_cmMode==='edit'&&_cmCourse){
      if(isAdmin()) body={action:'edit',adminKey:adminKey(),courseId:_cmCourse.id,name:name,km:_cmCourse.km,color:color};
      else body={action:'edituser',id:u.uid,tok:u.tok||'',courseId:_cmCourse.id,name:name,km:_cmCourse.km,color:color};
    }
    else {
      if(!_lastCourse) return;
      if(isAdmin()) body={action:'add',adminKey:adminKey(),name:name,color:color,coords:_lastCourse.coords,km:_lastCourse.km,segments:_lastCourse.segments||[]};
      else body={action:'adduser',id:u.uid,tok:u.tok||'',nick:u.nick||'',name:name,color:color,coords:_lastCourse.coords,km:_lastCourse.km,segments:_lastCourse.segments||[]};
    }
    const r=await fetch(base,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
    let saved={}; try{ saved=await r.json(); }catch(e){}
    if(r.ok){ gaEvent((_cmMode==='edit'||_cmMode==='editstatic')?'course_edit':'course_add'); bumpCourseVersion(); closeCourseModal(); map.closePopup();
      if(_cmMode==='edit'||_cmMode==='editstatic'){ location.reload(); }
      else { renderKVCourse(saved.course||{id:Date.now(),name:name,color:color,coords:_lastCourse.coords,km:_lastCourse.km,segments:_lastCourse.segments||[]}); _lastCourse=null; }
...
function staticWaterRoute(p1,p2){
  ...
  return _routeRiverPromise.then(function(fc){
    ...
    for(const f of (fc.features||[])){
      ...
      const score=a.dist+b.dist;if(!chosen||score<chosen.score)chosen={line:line,a:a,b:b,score:score,name:String((f.properties||{}).name||'')};
    }
...
async function waterRoute(p1,p2){
  ...
  const nodes={}, adj={}, nodeNames={}, toR=Math.PI/180;
  ...
  const waterOuter=[],waterInner=[],passLines=[];
  function addWater(list,pts){let s=90,w=180,n=-90,e=-180;pts.forEach(function(p){s=Math.min(s,p[0]);w=Math.min(w,p[1]);n=Math.max(n,p[0]);e=Math.max(e,p[1]);});list.push({p:pts,b:[s,w,n,e]});}
  function addPass(pts){addWater(passLines,pts);}
  function samePt(a,b){return a&&b&&Math.abs(a[0]-b[0])<1e-6&&Math.abs(a[1]-b[1])<1e-6;}
  function addClosed(list,pts){if(pts.length>3&&samePt(pts[0],pts[pts.length-1]))addWater(list,pts);}
  function addRelationRings(el,role,list){
    const pieces=(el.members||[]).filter(function(m){return (m.role||'outer')===role&&m.geometry&&m.geometry.length>1;}).map(function(m){return m.geometry.map(function(p){return [p.lat,p.lon];});});
    while(pieces.length){let ring=pieces.pop(),joined=true;while(joined&&!samePt(ring[0],ring[ring.length-1])){joined=false;for(let i=pieces.length-1;i>=0;i--){let p=pieces[i];if(samePt(ring[ring.length-1],p[0]))ring=ring.concat(p.slice(1));else if(samePt(ring[ring.length-1],p[p.length-1]))ring=ring.concat(p.slice(0,-1).reverse());else if(samePt(ring[0],p[p.length-1]))ring=p.slice(0,-1).concat(ring);else if(samePt(ring[0],p[0]))ring=p.slice(1).reverse().concat(ring);else continue;pieces.splice(i,1);joined=true;break;}}addClosed(list,ring);}
  }
  for(const el of (j.elements||[])){
    if(el.type==='way'&&el.tags&&el.tags.natural==='water'&&el.geometry&&el.geometry.length>2) addClosed(waterOuter,el.geometry.map(function(p){return [p.lat,p.lon];}));
    if(el.type==='relation'&&el.tags&&el.tags.natural==='water'){addRelationRings(el,'outer',waterOuter);addRelationRings(el,'inner',waterInner);}
    if(el.type==='way'&&el.tags&&el.geometry&&(el.tags.bridge||/^(weir|dam|lock_gate)$/.test(el.tags.waterway||'')||el.tags.man_made==='dam')) addPass(el.geometry.map(function(p){return [p.lat,p.lon];}));
    if(el.type!=='way'||!el.tags||!/^(river|stream|canal)$/.test(el.tags.waterway||'')||!el.geometry) continue; let prev=null;const wayName=String(el.tags.name||'');
    for(const p of el.geometry){ const k=key(p.lat,p.lon); if(wayName)(nodeNames[k]||(nodeNames[k]={}))[wayName]=((nodeNames[k]||{})[wayName]||0)+1;if(prev&&prev!==k){ const d=hav(nodes[prev],nodes[k]);
      (adj[prev]=adj[prev]||[]).push([k,d]); (adj[k]=adj[k]||[]).push([prev,d]); } prev=k; } }
  function inPoly(pt,poly){let inside=false;for(let i=0,j=poly.length-1;i<poly.length;j=i++){const a=poly[i],b=poly[j];if(((a[0]>pt[0])!==(b[0]>pt[0]))&&(pt[1]<(b[1]-a[1])*(pt[0]-a[0])/((b[0]-a[0])||1e-12)+a[1]))inside=!inside;}return inside;}
  function inWater(pt){let ok=false;for(const x of waterOuter){const b=x.b;if(pt[0]>=b[0]&&pt[0]<=b[2]&&pt[1]>=b[1]&&pt[1]<=b[3]&&inPoly(pt,x.p)){ok=true;break;}}if(!ok)return false;for(const x of waterInner){const b=x.b;if(pt[0]>=b[0]&&pt[0]<=b[2]&&pt[1]>=b[1]&&pt[1]<=b[3]&&inPoly(pt,x.p))return false;}return true;}
...
function _tourProjection(coords,p){
  ...
  let best=null,bestD=Infinity,along=0;
  for(let i=1;i<coords.length;i++){const a=coords[i-1],b=coords[i],ax=(a[1]-lng)*111320*cos,ay=(a[0]-lat)*110540,bx=(b[1]-lng)*111320*cos,by=(b[0]-lat)*110540,dx=bx-ax,dy=by-ay,t=Math.max(0,Math.min(1,-(ax*dx+ay*dy)/((dx*dx+dy*dy)||1))),x=ax+t*dx,y=ay+t*dy,d=Math.sqrt(x*x+y*y);if(d<bestD){bestD=d;best={seg:i-1,t:t,point:[a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t],alongM:along+segLen[i-1]*t,totalM:total,distM:d};}along+=segLen[i-1];}
  return best;
}
function _tourCourseSlice(coords,a,b){if(!a||!b)return[];if(a.alongM<=b.alongM)return[a.point].concat(coords.slice(a.seg+1,b.seg+1),[b.point]);return[a.point].concat(coords.slice(b.seg+1,a.seg+1).reverse(),[b.point]);}
...
function _tourAutoMotion(np,c){
  const t=_trk;if(!t||t.pauseType==='manual')return !t.paused;t.motion.push(np);t.motion=t.motion.filter(function(x){return np[2]-x[2]<=120000;});const first=t.motion[0],span=first?map.distance([first[0],first[1]],[np[0],np[1]]):0,threshold=Math.max(20,Math.min(45,(Number(c.accuracy)||20)*1.1)),moving=(Number(c.speed)>=.8)||(first&&np[2]-first[2]>=15000&&span>=threshold);
  if(t.paused&&t.pauseType==='auto'){t.moveHits=moving?t.moveHits+1:0;if(t.moveHits>=2){resumeTour(true);return true;}return false;}
...
async function openTripSummary(t){
  ...
  document.getElementById('tmCancel').onclick=function(){if(_pendTrip)[_pendTrip.line,_pendTrip.planLine,_pendTrip.estimateLine,_pendTrip.posMarker].forEach(function(l){if(l)map.removeLayer(l);});tripBackupClear();_pendTrip=null;closeTModal();}; }
async function saveTrip(){ if(!_pendTrip) return; const u=getUser(); if(!u||!u.uid) return;
  const title=(document.getElementById('tmTitle').value||'카누잉').trim().slice(0,40),shared=document.getElementById('tmShare').checked,saveAsCourse=!!(document.getElementById('tmSaveCourse')&&document.getElementById('tmSaveCourse').checked);
  ...
  try{ const r=await fetch(wapi('/trip'),{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({action:'save',id:u.uid,tok:u.tok||'',nick:u.nick||'',title:title,shared:shared,start:_pendTrip.startMs,end:_pendTrip.endMs,durSec:_pendTrip.durSec,track:_pendTrip.track,breaks:_pendTrip.breaks,gpsTrack:_pendTrip.gpsTrack,estimated:_pendTrip.estimated,courseId:_pendTrip.course&&_pendTrip.course.id||'',courseName:_pendTrip.course&&_pendTrip.course.name||'',courseProgress:_pendTrip.courseProgress,launch:_pendTrip.launch,landing:_pendTrip.landing})});
    if(r.ok){let courseFailed=false;if(saveAsCourse){const cr=await fetch(wapi('/course'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'adduser',id:u.uid,tok:u.tok||'',nick:u.nick||'',name:title,color:'#00897b',coords:_pendTrip.track.map(function(p){return[p[0],p[1]];}),km:_pendTrip.km})});courseFailed=!cr.ok;if(cr.ok)bumpCourseVersion();}gaEvent('trip_save',{km:Math.round(_pendTrip.km*10)/10,shared:shared});tripBackupClear();[_pendTrip.line,_pendTrip.planLine,_pendTrip.estimateLine,_pendTrip.posMarker].forEach(function(l){if(l)map.removeLayer(l);});_pendTrip=null;openTModal('trips');if(courseFailed)toastMsg('투어 기록은 저장했지만 코스 저장은 실패했어요');else if(saveAsCourse)toastMsg('투어 기록과 새 코스를 저장했어요');}
