...
function _riverSearch(q){
  ...
  names.forEach(function(n){groups[n].forEach(function(f){if(out.length>=10)return;const cs=f.geometry.coordinates,m=cs[Math.floor(cs.length/2)],region=_riverRegionLabel(m[1],m[0]);out.push({kind:'river',label:'🌊 '+n,sub:region+' · '+((f.properties||{}).kind==='river'?'강·하천':'지류 하천')+' · 전체 물줄기',lat:m[1],lng:m[0],name:n,features:[f]});});});return out;
}
...
function focusRiverSearch(x,keepUrl){
  ...
  (x.features||[]).forEach(function(f){const ll=f.geometry.coordinates.map(function(c){return [c[1],c[0]];});if(ll.length<2)return;if(!anchor)anchor=ll[Math.floor(ll.length/2)];L.polyline(ll,{color:'#001a33',weight:8,opacity:.68,interactive:false}).addTo(_riverSearchFocus);L.polyline(ll,{color:'#003b73',weight:4.5,opacity:1,interactive:false}).addTo(_riverSearchFocus);bounds.extend(ll);});
...
function shareCourse(idStr,name,km){ const u=courseShareUrl(idStr),info=_courseShareInfo(idStr,name,km),lines=['🛶 코스명: '+info.name];if(info.dist)lines.push('📏 거리: 약 '+info.dist+'km');lines.push('🔗 '+u);const full=lines.join('\n');gaEvent('course_share');
  if(navigator.share){ navigator.share({title:info.name+' | 마이카누',text:full}).catch(function(){}); }
