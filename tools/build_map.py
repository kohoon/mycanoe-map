...
function focusCourseFromUrl(){
  ...
  if(id.charAt(0)==='k'){ const kid=id.slice(1);   // KV 코스: 별도 조회
    fetch(courseReadUrl('/courses')).then(function(r){return r.json();}).then(function(list){ const c=(list||[]).find(function(x){return String(x.id)===String(kid);}); if(c){ _courseFocusFound=true; _applyCourseFocus(); _fitAndPop(c.coords, c.name, c.km); } }).catch(function(){});
    return; }
