...
function _riverSearch(q){
  ...
  return Object.keys(groups).sort(function(a,b){const ae=a.replace(/\s/g,'').toLowerCase()===nq,be=b.replace(/\s/g,'').toLowerCase()===nq;return ae===be?a.localeCompare(b,'ko'):(ae?-1:1);}).slice(0,8).map(function(n){
    const fs=groups[n],cs=fs[0].geometry.coordinates,m=cs[Math.floor(cs.length/2)]; return {kind:'river',label:'🌊 '+n,sub:(fs[0].properties.kind==='river'?'강·하천':'지류 하천')+' · 전체 물줄기',lat:m[1],lng:m[0],name:n,features:fs};
  });
}
...
function focusRiverSearch(x){
  ...
  (x.features||[]).forEach(function(f){const ll=f.geometry.coordinates.map(function(c){return [c[1],c[0]];});if(ll.length<2)return;L.polyline(ll,{color:'#063f52',weight:7,opacity:.55,interactive:false}).addTo(_riverSearchFocus);L.polyline(ll,{color:'#00c3df',weight:4,opacity:.95,interactive:false}).addTo(_riverSearchFocus);bounds.extend(ll);});
...
document.getElementById('srchForm').addEventListener('submit', async (ev)=>{
  ...
  const local=_localSearch(q).concat(_riverSearch(q));
