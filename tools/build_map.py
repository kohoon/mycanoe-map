...
function openCourseComments(key,course,shareId){
  ...
  if(course.coords && course.coords.length>=2){
    _cbtn='<a class="course-share" onclick="toggleCourseAnalysis()">📊 코스 분석</a>';
  }
  document.getElementById('pmLinks').innerHTML=(km?'<div class="pm-memo">약 '+km+'km</div>':'')
    +(shareId?'<a class="course-share" onclick="shareCourse(\''+shareId+'\')">🔗 코스 공유</a>':'')
    +_cbtn+_cadm
