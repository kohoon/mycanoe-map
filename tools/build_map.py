    ...
    .then(function(j){ const list=j.content||[]; if(!list.length) return;
      ...
      const items=list.slice(0,6).map(function(x){
        ...
        return '<button type="button" class="fld-item'+(has?'':' off')+'" data-lat="'+(has?s.lat:'')+'" data-lng="'+(has?s.lng:'')+'" title="'+pmEsc(z.type||'')+'">'
          ...
          +(has?'<em>핀이 표시됩니다</em>':'<em>좌표 없음</em>')+'</button>';
      }).join('');
      ...
      d.querySelectorAll('.fld-item').forEach(function(b){
        b.onclick=function(){
          ...
          if(isFinite(lat)&&isFinite(lng)){ showSearchPin(lat,lng,'홍수예보'); map.setView([lat,lng], 14); }
        };
      });
...
function finishMeasure(){
  ...
  if(isAdmin()){   // 관리자: 이 경로를 코스로 등록
    ...
    L.popup({closeButton:true}).setLatLng(end).setContent('<b>측정 '+km.toFixed(2)+'km</b><br><button class="addplace-btn" onclick="saveCoursePrompt()">💾 코스로 등록 <span class="beta-tag">BETA</span></button>').openOn(map);
  }
  ...
  if(wx){
    ...
    dailyCards=wx.days.slice(0,7).map(function(d){
      ...
      return '<div class="cm-ana-card cm-day-card"><div class="cm-ana-title">'+pmEsc(d.label||'')+'</div>'
        ...
        +'<div class="cm-ana-line"><b>'+lbl+'</b> · '+(dr===2?'강수/풍속이 높은 편':dr===1?'변동 가능성이 있음':'현재 조건상 안정적')+'</div></div>';
    }).join('');
  }
...
function openCourseModal(mode, course){
  ...
  document.getElementById('cmBody').innerHTML=
    '<h3>'+((mode==='edit'||mode==='editstatic')?'✏️ 코스 수정':'💾 코스 등록')+' <span class="beta-tag">BETA</span></h3>'
